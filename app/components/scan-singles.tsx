 ...
 * actual paid cost entered by the person scanning. Catalog value is display-only and never
 * becomes FIFO cost. New raw singles retain the confirmed listing for later market updates.
 */
export function ScanSinglesDialog({ onClose }: { onClose: () => void }) {
  ...
  async function saveMapping(productId: string, item: ScanItem) {
    if (!item.listing) return
    // A 409 means this product is already mapped. Stock is authoritative even if a
    // provider mapping cannot be added after the purchase succeeds.
  ...
  async function add(item: ScanItem, purchaseDate: string) {
    ...
    if (existing) {
      ...
      await saveMapping(existing.id, item)
      return
    }
    ...
    await saveMapping(product.id, item)
  }
  ...
  return <Sheet title="Scan singles" open compact onClose={onClose}>
    ...
    {games.data ? <>
      ...
      <Copy muted>
        TCGplayer estimates are display-only. Review each identity and enter what you actually
        paid per card; only that amount becomes purchase cost. Cards already on file get another purchase.
      </Copy>
