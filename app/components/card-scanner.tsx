...
import { Button, Copy, ErrorNotice, Row, styles as ui } from './ui'
...
export function CardScanner({ open, gameId, title, doneLabel, validate, onDone, onClose }: CardScannerProps) {
  const api = useApi()
...
function ScanRow({ item, disabled, onQuantity, onPrice, onRemove }: {
  item: ScanItem
  disabled: boolean
  ...
  return <View style={[ui.card, { padding: 12, gap: 6 }]}>
    <Row>
      ...
      <TextInput
        accessibilityLabel={`Price each for ${item.name}`}
        value={item.price}
        onChangeText={onPrice}
        editable={!disabled}
        keyboardType="decimal-pad"
        placeholder={item.status === 'pricing' ? '…' : '0.00'}
        placeholderTextColor={colors.muted}
        style={[ui.input, { minWidth: 84, minHeight: 40, padding: 8, textAlign: 'right' }]}
      />
