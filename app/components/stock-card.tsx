...
export function StockCard({ product: p, bucket, dense, onDetails, onSell, onMove, onEdit }: {
  product: Product; bucket: Bucket | ''; dense: boolean;
  onDetails: () => void; onSell: () => void; onMove: () => void; onEdit: () => void;
}) {
  ...
  return <Card><View role="group" accessibilityLabel={'Stock product: ' + p.name} style={{ flexDirection: dense ? 'row' : 'column', flexWrap: 'wrap', alignItems: dense ? 'center' : 'stretch', gap: 16 }}>
    <View style={{ flexGrow: 1, flexBasis: dense ? 260 : undefined, gap: 8 }}>
      <Row><View style={{ flex: 1, minWidth: 160 }}>
        ...
        <Copy muted>{p.product_type.name}{p.set_name ? ' · ' + p.set_name : ''}</Copy>
    ...
    <Row><Button label="Sell" disabled={noStock} onPress={onSell} />
      <Button label="Move" disabled={noStock} onPress={onMove} />
      <Button label="Edit" onPress={onEdit} /></Row>
  </View></Card>;
}
